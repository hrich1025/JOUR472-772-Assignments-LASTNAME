...
**Q2.** Of the departments listed in A1, which one is the most interesting to you, and why? **A2. University Honors has 21 classes listed, I'd be curious to know which those are and how they relate to climate.**
...
**A3. Business and Management by far has the most courses with a syllabus listed, while the next 4 are fairly similar in amount.**
...
**Q4.** Using the Maryland campaign expenses data, change the datatype of the expenditure_date column so that it is a date and not a chr, saving the result as a new dataframe. Then create a new column for the month based on expenditure_date. Then write code that shows total amount spent each month by Wes Moore's committee (you'll need to search the data to find the exact name, and we're not looking in payee_name). Write a sentence describing the pattern of spending over time. **A4.** Moore by far spent the most money in October 2022, while June and July followed as the next two highest.There appeared to be more spending at it got closer to November.

```{r}
standardized_maryland_expenses <- maryland_expenses |> 
  mutate(expenditure_date=mdy(expenditure_date),
         month = floor_date(expenditure_date, "month"),
         committee_name = str_squish(committee_name)
)

moore_monthly_spending <- standardized_maryland_expenses |>
...
**Q5.** Using case_when(), create a column in the Maryland expenses data called \`spending_location\` indicating whether each record spent money in Maryland or outside Maryland. For records that are in Maryland, make the new column's value "In-state" and for all others, make it "Out of state". Then write code that shows the total amount of money spent in each category and describe the results. You can do this in one statement or break it up into two statements (saving the new column to your dataframe). **A5.** There is significantly more money spent in-state than out of state, which makes sense.

**Q6.** Choose your own adventure: using either the UMD courses or campaign expenses data, ask and answer a question that is different than the previous ones. You can use one of the ones you provided in the pre_lab. **A6.** Which payee type had the most amount of expenditures?
