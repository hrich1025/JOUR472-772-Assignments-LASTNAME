4333--- title: "lab_06" author: "Derek Willis" date: "2025-01-16" output: html_document ---
...
allegany_911 <- read_csv("lab_06/data/allegany_911.csv") |> clean_names()
cecil_911    <- read_csv("lab_06/data/cecil_911.csv")    |> clean_names()
carroll_911  <- read_csv("lab_06/data/carroll_911.csv")  |> clean_names()
...
# load and join the Baltimore-related data
  baltimore_911 <- read_csv("lab_06/data/baltimore_911.csv") |> clean_names() |> mutate(zip_code = as.character(zip_code))
  zcta_demographics <- read_csv("lab_06/data/maryland_zcta.csv") |>  clean_names() |> mutate(zcta5n = as.character(zcta5n))
  
  baltimore_combined <- baltimore_911 |>
  left_join(zcta_demographics, by = c("zip_code" = "zcta5n"))
```
...
```{r}
combined_911 |> group_by(county) |> summarise(total_calls = n()) |> arrange(desc(total_calls))
```
...
A3. Zip code with the highest rate is 21217. Some of the neighborhoods there are Penn North, Harlem Park and Druid Heights. These are dense urban areas in West Baltimore.
...
A4: In Baltimore is there a relationship between the rate of overdose calls per 1,000 people and percentage of people under 18?
