...
```{r}
and_theatre_seats_1 <- umd_courses |> filter(department == "Theatre" & seats >= 15)
...
**Answer** 54,000 records of either courses with 15+ seats or courses in the Theatre department.
...
**Answer:**It's in the specific format of yyyy-mm-dd
...
**Answer**It's listing the first day of each month as that month.
...
**Answer**It's taking Moore's raw total divided the total number of votes to calculate his percentage. 
...
**Answer**It's showing Moore's percentages in each Zip Code in descending order.
...
**Answer**I asked ChatGPT: What are two tidyverse functions similar to str_to_upper? It said str_to_lowe, which converts text to all lowercase, and str_to_title, which converts text to title case.
...
**Answer**It's creating abbreviations for all the below states.
...
**Answer**Yes, it's grouping the total expenses by state, although the second-highest expensed state is NA
...
1.  Which month did the "Media" expense category have the most amount of expenditures?

2.  Which payee type had the most amount of expenditures?

3.  Could you mutate to abbreviate every Maryland town to figure out which had the most businesses with expenditures?
