...
**Task** Run the following codeblock to reload the data, using every row to set the data types. Does it show any parsing errors when you run? Answer below **Answer It does not show any parsing errors. Vendor Zip was moved to be a character.**
...
**Task** Run the following codeblock to glimpse the data. What data type is the "Vendor Name" field? What data type is the "Amount" field? What data type is the "Date" column? Answer below. **Answer Vendor name is a chr, amount is a dbl, date is a chr.**
...
**Task** Run the following codeblock and describe below what each line is doing. Looking at the results, what jumps out at you as potentially problematic? **Answer There are no listed loans in 2019, a random L column, a contract column and NAs.**
...
**Task** Run the following code to arrange the grants & loan data so that the smallest amount is first. Describe what you see. **Answer Some of the grantee names are all caps, some aren't; There are some 9 digit zip codes as well.**
...
**Task** Run the following codeblock to use the `clean_names()` function from janitor to standardize column names. How does it change the headers? Answer below. **Answer It made all the header names lowercase and put \_ between them instead of spaces,**
...
**Task** Run the following codeblock to use str_to_upper() to make the source, grantee and description columns upper-cased. Describe what each line is doing. **Answer Cleaned the header names, changed grantor to source and changed source, grantee and description to all upper case.**
...
**Task** Run the following codeblock to check for duplicate rows using get_dupes(). How many duplicates are possible? **Answer 580**
...
**Task** Run the following codeblock to use str_sub() to convert the ZIP codes that have nine digits to five digits, creating a new column. Look at the difference in the result - what changed? **Answer All the zip codes are 5 digits.**
...
**Task** Write and run code to answer our original question: which zip code has gotten the most amount of money from the Maryland Tourism Board? Where is that zip code, and does that zip code make sense to you as the top recipient? Write a sentence describing the top result. **Answer The zip code that received the most money from the Maryland Tourism Board is 21202, a zip code in downtown Baltimore covering the downtown and inner harbor areas. This would make a ton of sense as the top recipient of a tourism grant. It's one of the most densely populated areas in the state and receives tons of tourism. However, I feel as though the total amount I found, \$8.94 million, is quite a lot.**
